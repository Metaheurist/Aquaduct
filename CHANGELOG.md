...
### Video: Pro mode (frame-accurate slideshow)
- **Settings** ([`src/core/config.py`](src/core/config.py)): `VideoSettings.pro_mode`, `pro_clip_seconds`. Persisted in [`src/settings/ui_settings.py`](src/settings/ui_settings.py).
- **Pipeline** ([`main.py`](main.py)): when **slideshow** + **pro**, generates **`round(pro_clip_seconds × fps)`** images with the existing SDXL **reference chain**; skips per-frame quality retries; optional manifest key `pro_generated_frames`. Final assembly uses **`assemble_pro_frame_sequence_then_concat`** ([`src/render/editor.py`](src/render/editor.py)) — one still per output frame, narration **trimmed/padded** to `pro_clip_seconds` via FFmpeg (`AQUADUCT_PRO_MAX_FRAMES` env cap in `pro_mode_frame_count`).
- **UI** ([`UI/tabs/video_tab.py`](UI/tabs/video_tab.py), [`UI/main_window.py`](UI/main_window.py)): Pro checkbox, clip-length spin, hides **Images per video** while pro is on; forces slideshow when pro is enabled.
...
- **Preflight** ([`src/runtime/preflight.py`](src/runtime/preflight.py)): validates pro + slideshow; warns on large frame counts.
- **Tests**: [`tests/test_pro_mode_frames.py`](tests/test_pro_mode_frames.py); settings and preset tests extended. **Docs**: [`docs/config.md`](docs/config.md), [`docs/ui.md`](docs/ui.md), [`docs/editor.md`](docs/editor.md).
...
- **Platform template** at the top of **Video** uses **selectable tiles** (game-style preset cards): one tile per curated social profile (short-form vertical HD/720p, Instagram/Facebook square and 4:5, Pinterest 2:3, YouTube/LinkedIn 1080p, landscape 720p, etc.) plus **Custom** for full manual control. Definitions live in [`src/settings/video_platform_presets.py`](src/settings/video_platform_presets.py). Clicking a tile applies **resolution**, **FPS**, **micro-clip min/max**, **images per video**, **bitrate**, **clips per video**, and **seconds per clip**. The selection is stored as **`video.platform_preset_id`** in [`ui_settings.json`](src/settings/ui_settings.py) and surfaced in [`UI/main_window.py`](UI/main_window.py) as **`_video_platform_preset_id`**. Editing any of those fields switches the UI to **Custom**.
