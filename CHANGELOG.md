...
### Desktop UI
- **Topics → Discover** ([`src/content/crawler.py`](src/content/crawler.py), [`src/content/topics.py`](src/content/topics.py)): **News** and **Explainer** still use headline-style fallbacks (Google News RSS + MarkTechPost after Firecrawl). **Cartoon** and **Unhinged** use creative / story-shaped search strings and **skip** those headline sources; extra Firecrawl queries help fill the list ([`UI/tabs/topics_tab.py`](UI/tabs/topics_tab.py), [`UI/main_window.py`](UI/main_window.py)). Docs: [`docs/crawler.md`](docs/crawler.md), [`docs/ui.md`](docs/ui.md).
