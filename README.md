...
Alerts, confirmations, and most modal dialogs are **borderless** with a custom **close** icon and **rounded outline** buttons drawn like the main window title bar. **LLM chat** opens a non-modal window for the selected script model. **F12** toggles a **session-only** NSFW guardrail bypass **only when** `AQUADUCT_DEV_DISABLE_CONTENT_GUARDRAILS=1` was set at launch ([Desktop UI](docs/ui/ui.md)). API keys in `ui_settings.json` are **encrypted at rest**; `config show` redacts secrets by default (`--show-secrets` to opt in).
...
- **Tasks**: finished videos queue; live **stage + %** on the active row; **Pause** / **Stop** for long jobs; open/play, copy caption; **Approve** before auto-upload when enabled; **TikTok** and **YouTube** uploads when enabled (separate API toggles)
- **Library**: browse finished **video** or **photo** projects as **thumbnail cards** under **`.Aquaduct_data/videos`** or **`.Aquaduct_data/pictures`** (depending on **Photo \| Video**); **Advanced** adds **`runs/`** workspaces; **search** by title/folder/meta; **Resume series** for incomplete multi-episode folders; refresh or open the library roots
