...
- `advanced_tabs`: map of tab id → **`true`** when that tab’s **Advanced** mode is selected (e.g. `"pipeline"`, `"library"`, `"model"`). Omitted keys default to **Basic**. Persisted with Save; see [shared widgets — Basic \| Advanced](../ui/shared-widgets.md#basic--advanced-per-tab)
