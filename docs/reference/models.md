...
- **Script model (LLM)**: [`src/content/brain.py`](../../src/content/brain.py) (local inference; 4-bit target when supported). **Default** Hub id is **`Qwen/Qwen3-14B`** — the official Qwen3 14B chat/instruct repo (Qwen3 does **not** use a separate `-Instruct` suffix; the base repo is already instruction-tuned). Strong for creative and multi-turn work; on capable stacks you can use non-“thinking” / chat settings for fast prose where the runtime supports it). The **Model** tab lists four **curated** script options; you can still paste any compatible `AutoModelForCausalLM` repo id.
...
- **Video model**: [`src/render/clips.py`](../../src/render/clips.py) (motion: text-to-video; curated **Wan 2.2** / **Mochi 1** / **CogVideoX 5B** / **HunyuanVideo** / **LTX-2** plus user-typed ids such as SVD, ZeroScope, or LTX-Video). **Pro** with slideshow off can use this slot for **multi-scene text-to-video**. **Motion mode** (slideshow off, Pro off) can pair Image for keyframes + Video for **scene** segments. CLIP-77-style truncation applies only to **CLIP text** stacks, not to **CogVideoX** / **Wan** / **Mochi** / **Hunyuan** / **LTX** long-text paths; **SVD** img2vid (if used) has tighter **frame / decode** caps on **≤12 GiB** cards (see [Performance](../pipeline/performance.md#diffusion-vram-vs-system-ram-cpu-offload)).
...
- **API execution mode** (`model_execution_mode: api`): script / image / optional Pro video / voice are chosen from **Generation APIs** instead of the HF combos above. Defaults in the UI favor **Google AI Studio (Gemini)** (LLM), **SiliconFlow** (stills, Flux/SD3-class slugs), **Kling AI** (Pro *motion* / text-to-video, with **Magic Hour** and **Replicate** as alternates; free-tier notes in the [API generation doc](../integrations/api_generation.md)), and **Inworld** or **OpenAI** / **ElevenLabs** (voice). See [API generation](../integrations/api_generation.md) and [`src/runtime/pipeline_api.py`](../../src/runtime/pipeline_api.py).
