...
2. Generate a structured script package (LLM or fallback); optional **character** context from settings ([Characters](characters.md)). In **custom** mode, `src/brain.py` first expands instructions (`expand_custom_video_instructions`), then `generate_script(..., creative_brief=..., video_format=...)`. Optional LLM factcheck rewrite applies when article text exists (preset runs with fetched text)
