...
- **Title bar** (frameless window): drag by the title row; **Photo \| Video** ([`UI/media_mode_toggle.py`](../UI/media_mode_toggle.py)) switches **`media_mode`** (`video` \| `photo` in `ui_settings.json`) — output root **`.Aquaduct_data/videos`** vs **`.Aquaduct_data/pictures`**, tab set (**Video** vs **Picture**, Captions/Effects only in video mode), and Library rescan targets; **💾** saves settings; **📈** opens a live **resource usage** graph (this process CPU / RAM / GPU VRAM when CUDA is on, 1s updates); **?** opens the **Help** tutorial (topics on the left, slide-style pages with **Previous** / **Next**); **✕** closes the app. Icon buttons use [`UI/title_bar_outline_button.py`](../UI/title_bar_outline_button.py) (**`TitleBarOutlineButton`**) so outlines are **antialiased** (not jagged Fusion/QSS corners).
...
- **Run**
  ...
  - **Run** button: starts the pipeline. **Videos to generate** (`N > 1`) enqueues **N independent full runs** (each produces one video and one finished Tasks row); the first starts immediately and the rest wait in the FIFO queue. If a pipeline is **already running**, another click **queues** more runs (FIFO). Each queued job stores **settings** for one video (or preview/storyboard payloads) from when you clicked. **Stop** cancels the current job and **drops** the queue. While a run is active, **Run** stays enabled so you can enqueue more jobs.
