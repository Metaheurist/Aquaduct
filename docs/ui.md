...
- **Title bar** (frameless window): drag by the title row; **💾** saves settings; **📈** opens a live **resource usage** graph (this process CPU / RAM / GPU VRAM when CUDA is on, 1s updates); **?** opens the **Help** tutorial (topics on the left, slide-style pages with **Previous** / **Next**); **✕** closes the app.
...
- **Library**
  - Lists **`videos/`** project folders that contain **`final.mp4`** (title from **`meta.json`**, folder name, modified time, output size) and every subdirectory under **`runs/`** (pipeline workspaces; **assets/** column when present). **Refresh** rescans; opening the tab also refreshes; a completed pipeline run refreshes the lists.
  - **Open videos folder** / **Open runs folder** open **`.Aquaduct_data/videos`** or **`runs`** in the file manager. For a selected **videos/** row: **Open folder**, **Open assets** (under **`…/assets/`**), **Play final.mp4**; double-click opens the project folder. For **runs/**: **Open run folder** and **Open assets** when **`assets/`** exists ([`UI/tabs/library_tab.py`](../UI/tabs/library_tab.py), [`UI/library_fs.py`](../UI/library_fs.py)).
- **Video**
