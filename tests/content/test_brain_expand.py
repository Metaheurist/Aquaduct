...
def test_expand_custom_field_text_strips_code_fence(monkeypatch):
    from src.content.brain import api as b_api

    def fake_gen(model_id: str, prompt: str, **kwargs):
        return "```\nExpanded identity text here.\n```"

    monkeypatch.setattr(b_api, "_generate_with_transformers", fake_gen)
    out = b_api.expand_custom_field_text(model_id="dummy/Dummy", field_label="Identity", seed="host")
...
def test_expand_custom_video_instructions_returns_trimmed_text(monkeypatch):
    ...
    monkeypatch.setattr(b_api, "_infer_text_with_optional_holder", fake_infer)
    out = b_api.expand_custom_video_instructions(
...
def test_expand_custom_field_text_strips_quotes(monkeypatch):
    ...
    monkeypatch.setattr(b_api, "_generate_with_transformers", fake_gen)
    out = b_api.expand_custom_field_text(model_id="dummy/Dummy", field_label="Negatives", seed="")
