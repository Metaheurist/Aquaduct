...
def test_fetch_latest_prefers_firecrawl_when_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("src.firecrawl_news.firecrawl_search_news", fake_fc)
    monkeypatch.setattr("src.crawler._google_news_rss", boom_rss)
...
def test_fetch_latest_falls_back_to_rss_when_firecrawl_returns_nothing(monkeypatch: pytest.MonkeyPatch) -> None:
    from src.crawler import NewsItem, fetch_latest_items
    ...
    monkeypatch.setattr("src.firecrawl_news.firecrawl_search_news", fake_fc)
    monkeypatch.setattr("src.crawler._google_news_rss", rss)
    monkeypatch.setattr("src.crawler._marktechpost_latest", lambda **_k: [])
...
def test_fetch_article_text_falls_back_when_firecrawl_empty(monkeypatch: pytest.MonkeyPatch) -> None:
    from src.crawler import fetch_article_text

    monkeypatch.setattr("src.firecrawl_news.resolve_firecrawl_api_key", lambda _s: "key")
    monkeypatch.setattr("src.firecrawl_news.firecrawl_scrape_markdown", lambda *_a, **_k: "")
    ...
    monkeypatch.setattr("src.crawler.requests.get", lambda *_a, **_k: Resp())
...
def test_resolve_firecrawl_api_key_env_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    from src.firecrawl_news import resolve_firecrawl_api_key
