...
def subject_token_phrase_from_parts(gender: str, ethnicity: str, age_range: str) -> str:
    """Join identity tokens for storyboard / portrait anchoring (comma-separated)."""
    parts: list[str] = []
    g = (gender or "").strip()
    e = (ethnicity or "").strip()
    ar = (age_range or "").strip()
    if g:
        parts.append(g)
    if e:
        parts.append(e)
    if ar:
        parts.append(ar if ar.lower().startswith("age") else f"age {ar}")
    return ", ".join(parts)
...
class Character:
    ...
    visual_style: str = ""
    negatives: str = ""
    #: Free-form identity tokens for narration + storyboard anchoring (e.g. gender presentation, ethnicity, age band).
    gender: str = ""
    ethnicity: str = ""
    age_range: str = ""
    ...
    def from_dict(d: Any) -> Character | None:
        ...
        return Character(
            ...
            gender=_clip(str(d.get("gender", "")), 256),
            ethnicity=_clip(str(d.get("ethnicity", "")), 256),
            age_range=_clip(str(d.get("age_range", "")), 256),
...
def subject_token_phrase(ch: Character) -> str:
    return subject_token_phrase_from_parts(ch.gender, ch.ethnicity, ch.age_range)
...
def resolve_active_characters(settings: AppSettings) -> list[Character]:
    """Characters selected on the Run tab, in order (first = lead: voice + portrait reference)."""
    raw = getattr(settings, "active_character_ids", ()) or ()
    ids: list[str] = []
...
def resolve_active_character(settings: AppSettings) -> Character | None:
    chs = resolve_active_characters(settings)
    return chs[0] if chs else None


def merged_character_for_pipeline(chars: list[Character], *, video_format: str) -> Character:
    """
    Single synthetic :class:`Character` for script/TTS/storyboard when multiple Run-tab tags are selected.

    Lead (index 0) supplies voice overrides and portrait reference; identity aggregates the full cast.
    """
    if not chars:
        raise ValueError("merged_character_for_pipeline requires at least one Character")
    if len(chars) == 1:
        return chars[0]
    lead = chars[0]
    names = [c.name.strip() for c in chars if (c.name or "").strip()]
    cast_name = f"Cast: {names[0]} +{len(names) - 1}" if len(names) > 1 else (names[0] if names else "Cast")
    ...
    for i, c in enumerate(chars):
        ...
        if tok and role:
            ...
            head += f" ({role})"
        ident = (c.identity or "").strip()
        body = f"{head}\n  {ident}" if ident else head
        cast_lines.append(body)
        vs = (c.visual_style or "").strip()
        st = subject_token_phrase(c)
        vis = f"{st}, {vs}" if st and vs else (st or vs)
        if vis:
            visual_bits.append(vis)
        if (c.negatives or "").strip():
            neg_bits.append(c.negatives.strip())
        vi = (c.voice_instruction or "").strip()
        if nm and vi:
            voice_lines.append(f"- {nm}: {vi}")

    identity_block = (
        "Cast (mandatory):\n"
        + "\n".join(cast_lines)
        + "\n\nNarration must honor each cast member; keep identities distinct unless the format is single-host news.\n"
    )
    visual_joined = " | ".join(visual_bits)[:_MAX_FIELD]
    negatives = ", ".join(dict.fromkeys(neg_bits))[:_MAX_FIELD]
    if not negatives:
    ...
    voice_instruction = (
        ...
        if voice_lines
        else (lead.voice_instruction or "")
    )

    _ = video_format  # reserved for format-specific merge rules
    return Character(
        id=lead.id,
        name=_clip(cast_name, _MAX_NAME),
        identity=_clip(identity_block, _MAX_FIELD),
        visual_style=_clip(visual_joined, _MAX_FIELD),
        negatives=_clip(negatives, _MAX_FIELD),
        gender=lead.gender,
        ethnicity=lead.ethnicity,
        age_range=lead.age_range,
        reference_image_rel=lead.reference_image_rel,
        use_default_voice=lead.use_default_voice,
        pyttsx3_voice_id=lead.pyttsx3_voice_id,
        kokoro_voice=lead.kokoro_voice,
        voice_instruction=_clip(voice_instruction, _MAX_FIELD),
        elevenlabs_voice_id=lead.elevenlabs_voice_id,
    )
...
def resolve_character_for_pipeline(
    ...
    *,
    ...
    Uses active Run-tab character tag(s) when set (merged when multiple); otherwise generates a one-off
    in-memory character for this run (no disk write). Saved characters are only used when explicitly selected.
    """
    active = resolve_active_characters(settings)
    if active:
        return merged_character_for_pipeline(active, video_format=video_format)
...
def character_selected_in_settings(settings: AppSettings) -> bool:
    """True when the user explicitly picked one or more characters on the Run tab."""
    raw = getattr(settings, "active_character_ids", ()) or ()
...
def cast_to_ephemeral_character(*, cast: list[dict[str, Any]], video_format: str) -> Character:
    ...
    if vf in ("news", "explainer", "creepypasta", "health_advice"):
        ...
        g0 = _clip(str(c0.get("gender") or ""), 256)
        e0 = _clip(str(c0.get("ethnicity") or ""), 256)
        a0 = _clip(str(c0.get("age_range") or ""), 256)
        return Character(
            id=uuid.uuid4().hex,
    ...
    for c in safe[:4]:
        ...
        g = _clip(str(c.get("gender") or ""), 256)
        eth = _clip(str(c.get("ethnicity") or ""), 256)
        arng = _clip(str(c.get("age_range") or ""), 256)
        tok = subject_token_phrase_from_parts(g, eth, arng)
        if nm:
            head = f"- {nm}"
            extras: list[str] = []
            if tok:
    ...
    for c in cast:
        ...
        out.append(
            Character(
                ...
                gender=_clip(str(c.get("gender") or ""), 256),
                ethnicity=_clip(str(c.get("ethnicity") or ""), 256),
                age_range=_clip(str(c.get("age_range") or ""), 256),
                reference_image_rel="",
                use_default_voice=True,
                pyttsx3_voice_id="",
                kokoro_voice="",
...
def character_context_for_brain(ch: Character) -> str:
    ...
    tok_line_parts: list[str] = []
    if ch.gender.strip():
        tok_line_parts.append(ch.gender.strip())
    if ch.ethnicity.strip():
        tok_line_parts.append(ch.ethnicity.strip())
    if ch.age_range.strip():
        ar = ch.age_range.strip()
        tok_line_parts.append(ar if ar.lower().startswith("age") else f"age {ar}")
    if tok_line_parts:
        parts.append(
            "Identity tokens (must reflect in narration cues and every storyboard frame): "
            + "; ".join(tok_line_parts)
        )
