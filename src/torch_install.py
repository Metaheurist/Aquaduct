...
def _windows_branded_interpreter(role: str) -> str:
    ...
    that for pip subprocesses — each role gets a distinct name in Task Manager.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.download_all_windows_wheels is not None:
        ...
        try:
            from src.cli_pip_display import rich_windows_wheels_session, use_rich_cli
        except ImportError:
    ...
    try:
        from src.cli_pip_display import rich_pip_install_session, use_rich_cli
    except ImportError:
        use_rich_cli = lambda: False  # type: ignore[assignment, misc]
