...
def installation_dir() -> Path:
    ...
    # This file lives at ``src/core/app_dirs.py``; repo root is two levels up (not ``src/``).
    for up in (2, 1):
...
def _merge_misplaced_src_app_data(install: Path) -> None:
    """
    Older bug: ``installation_dir()`` pointed at ``src/``, so data landed in ``src/.Aquaduct_data``.
    Merge that tree into the real ``<repo>/.Aquaduct_data`` and remove the stray folder.
    """
    wrong = install / "src" / APP_DATA_DIR_NAME
    if not wrong.is_dir():
        return
    proper = install / APP_DATA_DIR_NAME
    if wrong.resolve() == proper.resolve():
        return
    proper.mkdir(parents=True, exist_ok=True)
    _merge_directories(wrong, proper)
    try:
        shutil.rmtree(wrong)
    except OSError:
        pass
...
def _run_migration_once() -> None:
    ...
    _merge_misplaced_src_app_data(install)
